...
- **영향**: `bridge/`의 요청/응답 규약(Phase 1)은 이 틀을 기준으로 설계한다. 정확한 필드 스키마는 Phase 1에서 확정한다. D-003의 "매 턴 보드 상태"가 원문(raw)인지 이미 encoding된 텐서인지는 아래 미결정 항목에 달려 있다.

## 미결정: encoding을 Rust와 Python 중 어디서 하는가
...
- **영향**: Phase 6(state/action encoding) 착수 전에 반드시 해결해야 한다. 확정되면 이 항목을 정식 D-XXX로 승격한다.
